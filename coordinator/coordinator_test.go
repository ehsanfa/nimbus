...
func TestCoordinatorSet(t *testing.T) {
	...
	defer l.Close()
	self := cluster.NewNode("localhost:9091", l.Addr().String(), []partition.Token{12}, cluster.NODE_STATUS_OK)
	...
	coord, err := cp.GetClient(self.DataStoreAddress)
	if err != nil {
		panic(err)
	}
...
func BenchmarkCoordinatorPrepare(b *testing.B) {
	addr1 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-9001)+9001)
	addr2 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-9001)+9001)
	addr3 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-9001)+9001)
	addr4 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-9001)+9001)
	ctx := context.Background()
	cp := connectionpool.NewConnectionPool(connectionpool.NewTcpConnector())
	stg := storage.NewDataStore(ctx)
	node1 := cluster.NewNode(addr1, addr2, []partition.Token{12}, cluster.NODE_STATUS_OK)
	...
	node2 := cluster.NewNode(addr3, addr4, []partition.Token{12}, cluster.NODE_STATUS_OK)
...
func BenchmarkCoordinatorSet(b *testing.B) {
	addr1 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr2 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr3 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr4 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr5 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr6 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr7 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	addr8 := fmt.Sprintf("127.0.0.1:%d", rand.Intn(9850-7001)+7001)
	ctx := context.Background()
	cp := connectionpool.NewConnectionPool(connectionpool.NewTcpConnector())
	stg := storage.NewDataStore(ctx)
	node1 := cluster.NewNode(addr1, addr2, getPartitions(256), cluster.NODE_STATUS_OK)
	clstr1 := cluster.NewCluster(node1, 3, cluster.CONSISTENCY_LEVEL_ALL)
	ds := datastore.NewDataStore(ctx, stg, clstr1, cp)
	c := NewCoordinator(ctx, clstr1, ds, stg, "127.0.0.1:")

	node2 := cluster.NewNode(addr3, addr4, getPartitions(256), cluster.NODE_STATUS_OK)
	clstr2 := cluster.NewCluster(node2, 3, cluster.CONSISTENCY_LEVEL_ALL)
	datastore.NewDataStore(ctx, stg, clstr2, cp)

	node3 := cluster.NewNode(addr5, addr6, getPartitions(256), cluster.NODE_STATUS_OK)
	clstr3 := cluster.NewCluster(node3, 3, cluster.CONSISTENCY_LEVEL_ALL)
	datastore.NewDataStore(ctx, stg, clstr3, cp)

	node4 := cluster.NewNode(addr7, addr8, getPartitions(256), cluster.NODE_STATUS_OK)
